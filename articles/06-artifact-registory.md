...
- Webアプリのビルドプロセスを自動化し、成果物をDockerイメージとしてArtifact Registryに保存する。
- GCPを使用したコードのバージョン管理と、そのセットアップ手順の記録。

# Artifact Registryとは？

Artifact Registryは、コンテナイメージや言語ベースのパッケージを一元管理するGCPのサービス。プライベートなアーティファクトの安全なホスティング、共有、管理が可能。

主な特徴は以下の通り。

- 一元管理: 複数のリポジトリタイプを同じ場所で管理できる（例：Docker, npm, Maven, etc.）。
- セキュリティ: 組織のパッケージはプライベートな環境で安全にホストされる。
- 統合: CI/CDワークフローとのシームレスな統合が可能。
- 拡張性: 大規模なプロジェクトや組織の成長に合わせてスケールすることができる。
...
4. 「作成」をクリックしてリポジトリを作成する。

# 🔐 Artifact Registry用のGCPのサービスアカウントを作成する

Artifact Registryを操作するサービスアカウントを作成する。
...
5. サービスアカウントのキーを作成する。
6. GitHubのリポジトリを作成し、シークレットにキー情報を保存する。

# Next.jsアプリの構築

以下のテクノロジースタックを使用したサンプルWebアプリを構築する。
...
`docker\Dockerfile`のファイル内容は以下となる。
...
Dockerコンテナ起動時に実行する `scripts\entrypoint\entryPoint.sh`は以下とする。
...
GitHub Actionsで実行するワークフロー(.github\workflows\build-and-deploy.yml)は以下とする。
このワークフローは、GitHubリポジトリにプッシュされる度にトリガーされ、新しいDockerイメージがArtifact Registryにプッシュされる。
...
この記事では、WebアプリのDockerイメージを自動的にビルドし、GCPのArtifact Registryに保存するプロセスを設定した。これにより、開発チームはアプリケーションのデプロイメントを簡単かつ迅速に行うことができ、バージョン管理を効果的に行うことが可能となる。
